"""복리후생 상담의 최종 답변을 고정하는 회귀 테스트입니다.

여기 남는 것은 LLM 답변을 실제로 봐야 하는 사례뿐입니다. 검색만으로 판정되는 것(근거
파일 구성, 종목어 도달)은 eval_retrieval.py로 옮겼습니다. 그쪽은 LLM을 부르지 않아 5초에
끝나므로 노브를 바꿀 때마다 함께 돌릴 수 있는데, 이 파일은 사례마다 LLM을 부르므로 100초가
걸리고 회차마다 1~3건이 흔들립니다. 실패를 보면 회귀로 단정하기 전에 그 사례만 3회 다시
...
PROTECTED_CASES = [
    # 경조금 카드: 개인 사유를 되묻지 않고 규정의 네 항목 전체를 안내한다.
    (
        "경조금 지원 기준에서 지급 대상, 금액, 신청 기한과 필요 서류를 알려줘.",
        ("1. 지급 대상", "출산장려금", "2,000,000원", "3개월 이내", "4. 필요 서류", "기본증명서"),
        "경조사 사유 또는 관계",
    ),
    (
        "출산지원금 알려줘",
        ("첫째 3,000,000원", "둘째 7,000,000원", "셋째 이상 10,000,000원", "3개월 이내", "출생증명서"),
        ("금액은 제공된 규정에 없음", "신청기한과 구체적인 지급액은 제공된 규정에 없어"),
    ),
    (
    ...
    # 옛 기대값 '회사에서 지원하지 않습니다'는 삭제된 build_overseas_personal_return_answer가
    # 만들던 문장이다. 생성 답변은 주어를 넣기도("회사에서 지원하지 않습니다") 빼기도
    # ("지원되지 않습니다") 한다. 5회를 뽑아 보면 판정은 매번 지급 불가로 같고 금지 문자열도
    # 나오지 않는다. 주어에 기대지 않는 표현으로 확인한다.
    (
...
def check_missing_chips():
    ...
    state = {
        ...
        "candidate_evidence": [{"file": "숙소지원금 운영 기준.md"}],
